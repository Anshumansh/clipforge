...
**A second real CI-only defect, found chasing the first one properly rather than declaring victory early:** after the entrance-fade-timing fix (`4434c55`) resolved `/how-it-works`, `/trust`, and `/contact`, `/vs/opus-clip` and `/vs/revid-ai` kept failing — but *consistently* (10/10 repeated runs on `mobile-chrome`), which is a different signature than the flaky 4-out-of-however-many pattern the first defect had. That distinction mattered: it meant this was a real, deterministic bug, not the same flakiness with more margin needed. A first hypothesis (the page's continuous `.ambient-glow` background animation) was tested and ruled out (pausing all CSS animations didn't change the result). The actual cause, found by hand-computing contrast for the real candidate: `components/comparison-landing.tsx`'s losing/neutral comparison-row icons rendered at `text-muted-foreground/50` (50% opacity) — genuinely ~3.34:1 against this theme's background, under the 4.5:1 floor, every time, in any browser. Fixed to full-opacity `text-muted-foreground` (already ~10.6:1, already used elsewhere on the same page). Commit `3e23abc`. CI run for this fix in progress at time of writing.
