...
## 10. Resolution — real root cause found, permanent-showcase architecture landed, two consecutive green runs (2026-08-25)

The "unresolved, provider-side" conclusion in section 9 was **wrong on the root cause**, corrected by direct evidence, not assumption. Instructed to stop inferring from indirect signals (curl testing, pattern-matching) and instead download and inspect the actual CI trace data from a failing run before drawing any further conclusion.

**Root cause, proven from a downloaded trace, not inferred:** extracted the full `0-trace.network` record for the failing "Script to video" request from run `32828097747` (chromium and mobile-chrome, identical). The request was to `clipforge-v2-staging.up.railway.app/api/media/jobs/.../output.mp4` — status 404, `server: railway-hikari` (Railway's own edge, **not** Tigris), `content-type: application/json` matching `/api/media/[...key]`'s own `NextResponse.json` 404 body, and **no redirect ever occurred**. The request never reached the storage provider. This definitively separates two distinct issues that had been conflated:
1. The genuine, Tigris-side `AccessDenied` from section 9's abandoned copy-to-permanent-key attempt (real, but irrelevant to this failure).
2. The actual cause of *this* failure: `app/page.tsx` had no dynamic APIs, so Next statically prerendered it once at Docker build time with no real `.env` available (confirmed by the code's own pre-existing comment, which had already diagnosed this exact mechanism for a different env var earlier in this pass) — including `STORAGE_*`, causing `getPresignedDownloadUrl` to return null and the homepage to fall back to the ownership-gated media proxy, which correctly 404s for an anonymous visitor. ISR (`revalidate=300`) only regenerates in the background *after* a request lands past the window, so the first hit after any deploy — including every CI run, which starts testing within seconds — reliably got the stale, secrets-less build-time render.

**Fix:** `export const dynamic = "force-dynamic"` on `app/page.tsx` (commit `35ae55e`), eliminating this entire class of bug outright rather than patching each build-time-unavailable value as it's discovered. Verified with a full local `next build` (after fixing an unrelated stale local `node_modules` — Next 15 was installed despite `package.json` pinning `^14.2.35`; `npm ci` corrected it): homepage now builds as `ƒ` (dynamic) instead of `○` (static).

**Owner landed a comprehensive parallel commit** (`9111a20`, `1f94be1`, `46de3be`) on top of this fix, pushed directly to `recovery/full-functional` while this session was working the same branch — discovered via a routine `git fetch` before an intended push, reconciled by fast-forwarding (clean linear history throughout, zero conflicts) rather than overwriting. Reviewed and verified rather than duplicated:
- **`/api/showcase/[name]`** (`script`/`repurpose`/`ugc` only, strict allowlist): streams video server-side through authenticated storage SDK calls — the client never touches Tigris directly, sidestepping the section-9 provider-side issue entirely regardless of its actual cause. Supports GET/HEAD/Range, correct `Content-Type`/`Content-Length`/`Accept-Ranges`/`Content-Range`/`ETag`/`Cache-Control`, never exposes bucket name or credentials. Storage key is env-var-overridable per name with a safe fallback to the exact job-scoped keys already proven working, and path-traversal-checked.
- CI preflight rewritten to HEAD + Range-GET all three `/api/showcase/*` endpoints directly (same-origin, never the provider's domain) before the browser matrix starts.
- `ClipTile`'s `.play()` now fires synchronously inside the triggering event (element permanently mounted, `src` empty until interaction) instead of via a `requestAnimationFrame` deferral — a real cross-browser autoplay fix, not just a test workaround.
- Real, unrelated billing fixes: plan selection preserved across signup, checkout cancellation/continuation states, network/provider failure handling, Stripe return URLs derived from configured origin.
- Dashboard-navigation simplification and a guided `/dashboard/create` hub (the `ux/simple-user-journey` ask).

**This session's remaining contribution once synced to that work:** two more real, narrow bugs surfaced by real CI runs and fixed in place:
- `response.headers()["content-type"]` came back `undefined` on WebKit (commit `9995e42`, superseded) — WebKit's Playwright adapter doesn't reliably normalize/preserve header casing.
- The owner's own follow-up (`46de3be`) went further and removed the browser-side content-type check entirely as redundant with the CI preflight's own dedicated verification, relying on the video element's own stronger signals (`currentSrc`, `readyState`, `duration`, `error`, `currentTime` advancing) instead — the correct call, adopted over this session's own intermediate fix.

**Verified: two consecutive fully green CI runs**, same commit `46de3be011aba29ac96e21431fa0cf3fee55a767`, all four browsers both times — https://github.com/Anshumansh/clipforge/actions/runs/32835291257 (attempt 1 was the first fully green run this entire session; attempt 2 is a `gh run rerun` of the identical commit, both attempts viewable from that same run page, confirming it wasn't a fluke).

**Production confirmed untouched:** `origin/main` unchanged at `0cf6ba0` throughout; `recovery/full-functional`'s HEAD is not an ancestor of `main` (`git merge-base --is-ancestor` false). The real, customer-facing production (VPS at `forgecut.app`, deployed only via `deploy.yml`'s push-to-main-gated SSH step) was never reachable by anything this pass did. One observation flagged for the owner, not an incident: the Railway project's environment literally labeled `production` (id `26f39e2c-bbf1-4957-9cc0-9612579c1bf3`) has **no domain attached** — it is not the real production, appears to auto-deploy from the same source as `staging`, and poses no actual exposure since no real user can reach it, but the naming is worth reviewing so it's never confused with the real thing later.

**Files changed this session, in the order landed:** `app/api/version/route.ts` (+test), `.github/workflows/e2e.yml` (SHA-preflight gate, later the showcase-endpoint check), `app/page.tsx` (Repurpose/UGC key repair, then `force-dynamic`), `tests/e2e/anonymous-journey.spec.ts` (real `<video>`-state assertions), `lib/storage.ts` (briefly: `objectExists`/`copyObject`, fully reverted), `components/hero-demo.tsx` (synchronous `.play()`), `RECOVERY_EVIDENCE.md`. Owner's parallel commits touched `app/api/showcase/[name]/route.ts` (+test), `lib/showcase-assets.ts` (+test), `lib/storage.ts` (`getStoredObject`/`headStoredObject`), `app/dashboard/create/page.tsx`, `components/dashboard-nav.tsx`, `app/login/page.tsx`, `middleware.ts` (+test), `app/api/stripe/checkout/route.ts`, `app/api/stripe/portal/route.ts`, `lib/public-app-url.ts` (+test), `lib/safe-redirect.ts` (+test), and others per the stat above.
