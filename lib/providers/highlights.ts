...
function clampClips(clips: HighlightClip[], durationSec: number): HighlightClip[] {
  ...
  // Drop clips that overlap a previously accepted one.
  ...
  return nonOverlapping.slice(0, 4);
}
    ...
    [
      {
        ...
        content:
          ...
          `timestamps from the transcript, and not overlap other clips. Return strict JSON: ` +
          `{"clips": [{"startSec": number, "endSec": number, "title": string (<=6 words, no clickbait)}]}.`,
      },
